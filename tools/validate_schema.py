...
def default_schema_dir():
    ...
    官方模板把 schema 放在 tools/schema；本项目沿用上游布局放在 deps/tools
    （那是从 MaaFramework 同步过来的那一份），所以自动探一下，别让人手敲参数。
    """
    for candidate in ("tools/schema", "deps/tools"):
        if Path(candidate, "pipeline.schema.json").is_file():
            return candidate
    return "tools/schema"
...
def main():
    if hasattr(sys.stdout, "reconfigure"):
        # Windows 终端默认 GBK，下面的 ✓ / ❌ 会直接抛 UnicodeEncodeError
    ...
    parser.add_argument(
        ...
        help="Directory containing schema files (default: 自动探测 tools/schema 或 deps/tools)",
    )
    parser.add_argument(
        "--resource-dirs",
        type=str,
